...
if ([string]::IsNullOrWhiteSpace($PackagePath)) {
    $latest = Get-ChildItem -Path $root -Filter "Warzone_M13_source_*.zip" -File -ErrorAction SilentlyContinue | Sort-Object LastWriteTime -Descending | Select-Object -First 1
    if ($null -eq $latest) {
        Write-Output "HANDOFF_PACKAGE_CHECK: SKIPPED (no M13 handoff package found)"
        exit 0
    }
